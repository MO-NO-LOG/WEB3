...
fetch(API + "/api/movies/trend")
  .then((res) => res.json())
  .then((movies) => {
    document.getElementById("trendList").innerHTML = movies.map((m) => renderMovieCard(m)).join("");
  });
...
fetch(API + "/api/movies/recommended?limit=30")
  ...
  .then((movies) => {
    ...
    document.getElementById("ratingList").innerHTML = sorted.map((m) => renderMovieCard(m)).join("");

    document.getElementById("endingList").innerHTML = movies.map((m) => renderMovieCard(m)).join("");
  });
